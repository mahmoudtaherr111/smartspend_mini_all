...
async function migrate() {
  ...
  try {
    ...
    try {
      ...
      await connection.query(
        "ALTER TABLE `users` ADD COLUMN `highest_streak` int DEFAULT 0",
      );
      await connection.query(
        "ALTER TABLE `users` ADD COLUMN `last_streak_at` datetime",
      );
    ...
    try {
      ...
      await connection.query(
        "ALTER TABLE `local_users` ADD COLUMN `current_streak` int DEFAULT 0",
      );
      await connection.query(
        "ALTER TABLE `local_users` ADD COLUMN `highest_streak` int DEFAULT 0",
      );
      await connection.query(
        "ALTER TABLE `local_users` ADD COLUMN `last_streak_at` datetime",
      );
