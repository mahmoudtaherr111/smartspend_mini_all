...
Ensure `Authorization: Bearer ${token}` and cookie session handling remain intact and functional. Clean standard headers should be sent without injecting unwanted custom tunnel bypass headers. The `headers()` function should return only `{ Authorization: `Bearer ${token}` }` when a token exists, and `{}` when not authenticated.
...
1. Dispatch to teamwork_preview_implementer in a clean directory (e.g. E:\smartspend_V1_fixed\.agents\swe_4_implementer).
2. Run review loops with teamwork_preview_reviewer.
3. Maintain plan.md/progress.md/BRIEFING.md in your working directory.
4. When complete, send a completion message back to the Sentinel with your final summary report.
