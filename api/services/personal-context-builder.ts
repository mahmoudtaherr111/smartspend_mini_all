 ...
 */
export function buildPersonalContext(profile: SmartUserProfile): PersonalContext {
  const lifestyle = profile.lifestyleInfo as Record<string, any>;
  const knownPeople: KnownPerson[] = [];

  // Children names
  const childrenNames: string[] = Array.isArray(lifestyle.childrenNames) ? lifestyle.childrenNames : [];
  ...
  const contacts: string[] = Array.isArray(lifestyle.regularContacts) ? lifestyle.regularContacts : [];
  ...
  const supportsOthers: string[] = Array.isArray(lifestyle.supportsOthers) ? lifestyle.supportsOthers : [];
  ...
  for (const key of supportsOthers) {
    if (supportsMap[key] && key !== "none") {
      // These don't have names, but we track the relationship type
      // They enhance AI context without specific name matching
    }
  }

  // Pet names (for fun context)
  const petNames: string[] = Array.isArray(lifestyle.petNames) ? lifestyle.petNames : [];
  ...
  const subscriptions: string[] = Array.isArray(lifestyle.subscriptions) ? lifestyle.subscriptions : [];
  const places: string[] = Array.isArray(lifestyle.favoriteSpendingPlaces) ? lifestyle.favoriteSpendingPlaces : [];
...
export function buildPersonalContextPrompt(ctx: PersonalContext): string {
  ...
  if (ctx.knownPeople.length > 0) {
    ...
    for (const person of ctx.knownPeople) {
      lines.push(`  - ${person.name} (${person.relationship}) → إذا ذُكر اسمه، صنّف كـ "${person.category} > ${person.subCategory}"`);
    }
    lines.push(`تعليمة: إذا قال المستخدم "بعتت فلوس لـ[اسم]" أو "أديت [اسم]" واسم الشخص من القائمة أعلاه، اذكر العلاقة في الوصف. مثلاً: "تحويل لابنك محمد" بدلاً من "تحويل".`);
  }

  if (ctx.knownSubscriptions.length > 0) {
    const subMap: Record<string, string> = {
      netflix: "Netflix", shahid: "شاهد VIP", spotify: "Spotify",
      youtube_premium: "YouTube Premium", gym: "جيم/نادي",
      internet: "إنترنت منزلي", phone_plan: "باقة موبايل",
      insurance: "تأمين",
    };
  ...
  if (ctx.hasCar) {
    lines.push(`لديه سيارة${ctx.carType ? ` (${ctx.carType})` : ""} — أي ذكر لبنزين/صيانة/غسيل عربية يصنف تلقائياً كـ "مواصلات > سيارة خاصة"`);
  }
  ...
  if (ctx.salaryDay && ctx.salaryDay > 0) {
    ...
    lines.push(`تعليمة: لو المستخدم ذكر "المرتب" أو "القبض" أو "الراتب"، صنّفه كـ "دخل" وتاريخه يوم ${ctx.salaryDay} من الشهر الحالي.`);
  }
...
export function buildFamilyReportContext(ctx: PersonalContext): string {
  ...
  const contacts = ctx.knownPeople.filter(p => p.relationship === "شخص معروف");
  ...
  if (contacts.length > 0) {
    lines.push(`أشخاص يحول لهم بانتظام: ${contacts.map(c => c.name).join("، ")}`);
  }
