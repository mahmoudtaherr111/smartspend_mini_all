...
export const adsRouter = router({
  ...
  impression: publicProcedure
    ...
    .mutation(async ({ input }) => {
      await db.update(ads).set({ impressions: sql`impressions + 1` }).where(eq(ads.id, input.adId));
      return { success: true };
    }),
  ...
  click: authedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await db.update(ads).set({ clicks: sql`clicks + 1` }).where(eq(ads.id, input.adId));
  ...
  stats: adminProcedure.query(async () => {
    ...
    const totalImpressions = await db.select({ sum: sql`SUM(impressions)` }).from(ads);
    return { ads: allAds, totalClicks: totalClicks[0]?.sum ?? 0, totalImpressions: totalImpressions[0]?.sum ?? 0 };
  }),
});
