...
export function redactSensitiveData(text: string): string {
  ...
  r = r.replace(/account ending with \d{4}/gi, "account ending with [REDACTED_CARD]");
  ...
  const otpKeywords = /(?:رمز|كود|التحقق|برقم|otp|code|verification|passcode|pin)(?:[^\d]{1,30})?(\b\d{4,6}\b)/gi;
  r = r.replace(otpKeywords, (match, p1) => match.replace(p1, "[REDACTED_OTP]"));
  ...
  r = r.replace(/(?:إلى|الى|من|حساب|لرقم|لرقم محفظة)\s+([A-Za-z\s]{3,25})(?=\s+(?:عبر|بنجاح|via|through|$))/g, (match, p1) => {
    const name = p1.trim();
    if (/^(instapay|vodafone|orange|etisalat|cib|nbe|qnb|bank|egp|le|usd|eur)$/i.test(name)) {
      return match;
    }
    return match.replace(p1, "[REDACTED_NAME] ");
  });

  r = r.replace(/(?:to|from)\s+([A-Za-z\s]{3,25})(?=\s+(?:via|through|successfully|$))/gi, (match, p1) => {
    const name = p1.trim();
    if (/^(instapay|vodafone|orange|etisalat|cib|nbe|qnb|bank|egp|le|usd|eur)$/i.test(name)) {
      return match;
    }
