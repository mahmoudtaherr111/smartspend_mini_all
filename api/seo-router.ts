...
export const seoRouter = router({
  ...
  getPage: publicProcedure
    ...
    .query(async ({ input }) => {
      const page = await db.select().from(seoPages).where(eq(seoPages.path, input.path)).limit(1);
      if (page[0]) return page[0];
      // Default SEO
      return {
        path: input.path,
        title: "SmartSpend AI - تتبع المصاريف بالذكاء الاصطناعي",
        description: "تطبيق ذكي لتتبع المصاريف اليومية باللهجة المصرية. حلل مصاريفك بالصوت أو الكتابة واحصل على نصائح مالية ذكية.",
  ...
  upsert: adminProcedure
    ...
    .mutation(async ({ input }) => {
      const existing = await db.select().from(seoPages).where(eq(seoPages.path, input.path)).limit(1);
      if (existing[0]) {
        await db.update(seoPages).set(input).where(eq(seoPages.id, existing[0].id));
      } else {
  ...
  sitemap: publicProcedure.query(async () => {
    const pages = await db.select({ path: seoPages.path, updatedAt: seoPages.updatedAt }).from(seoPages);
