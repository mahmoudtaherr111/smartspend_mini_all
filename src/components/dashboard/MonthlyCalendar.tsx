...
export const MonthlyCalendar = memo(function MonthlyCalendar({ month, dayTrend = [], salaryDay }: { month: string; dayTrend?: any[], salaryDay?: number }) {
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  amount > 0 && "bg-red-50 border-red-100 dark:bg-red-950/20 dark:border-red-900"
                )}
                style={amount > 0 ? { opacity: 0.55 + intensity * 0.45 } : undefined}
              >
                {!cell.empty && (
                  <>
                    <div className="font-semibold text-xs sm:text-sm mb-1">{Number(cell.day)}</div>
                    <div className="mt-auto space-y-0.5">
                      {amount > 0 && (
                        <div className="text-[9px] sm:text-[11px] font-bold text-red-700 dark:text-red-400 truncate leading-tight" title={`${amount} ج.م`}>
                          {compactMoney(amount)}<span className="hidden sm:inline"> ج</span>
                        </div>
                      )}
                      {Number(cell.data?.income || 0) > 0 && (
                        <div className="text-[9px] sm:text-[11px] font-bold text-emerald-700 dark:text-emerald-400 truncate leading-tight" title={`دخل ${cell.data.income}`}>
