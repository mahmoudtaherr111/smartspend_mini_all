...
export function NotificationBell() {
  ...
  const { data: notifications } = trpc.profile.getInAppNotifications.useQuery(
    undefined,
    {
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="relative text-white/70 hover:text-white hover:bg-white/10 dark:text-white/70 dark:hover:bg-white/10 text-slate-700 hover:bg-slate-100"
          aria-label={
            unreadCount > 0
          ...
          notifications.map((notif) => (
            ...
            >
              <div className="flex justify-between w-full items-center">
                <span
                  className={`font-semibold text-sm ${!notif.isRead ? "text-indigo-700 dark:text-indigo-400" : ""}`}
                >
                  {notif.title}
                </span>
                {notif.createdAt && (
