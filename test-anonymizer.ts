...
const testCases: AnonymizerTestCase[] = [
  ...
  {
    description:
      "Egyptian mobile phone number redaction (international format)",
  ...
  },
  {
    ...
    input:
      "CIB Account transfer details: 5082736481029384 transaction complete.",
...
for (let i = 0; i < testCases.length; i++) {
  ...
  if (success) {
    ...
    if (missingContains.length > 0) {
      console.log(
        `   Missing expected tags: ${JSON.stringify(missingContains)}`,
      );
    }
    if (leakedContains.length > 0) {
      console.log(
        `   Leaked sensitive data: ${JSON.stringify(leakedContains)}`,
      );
    }
  }
}
...
if (passed === testCases.length) {
  console.log(
    "🎉 ALL ANONYMIZER TESTS PASSED SUCCESSFULLY! The privacy engine is fully secure.",
  );
} else {
