...
### نظام القراءة الآلية للرسائل (Automated SMS Ingestion)

يسمح هذا النظام بالتقاط الرسائل البنكية تلقائياً وتحويلها فوراً إلى دخل أو مصروف في النظام دون أي تدخل من المستخدم. 

#### كيف يعمل؟ (الربط مع المصروفات والدخل)
1. **التقاط الرسالة:** يقرأ النظام محتوى الرسالة، فإذا وجد أنها **تحويل وارد**، **إيداع**، أو **قبض راتب**، يقوم بتسجيل العملية فوراً تحت نوع `Income` (دخل) بقيمة موجبة. أما إذا كانت **خصم** أو **سحب** أو **عملية شراء** (بما في ذلك **Apple Pay**)، فتسجل כـ `Expense` (مصروف) يتم طرحه من إجمالي الميزانية.
2. **الربط مع الإحصائيات:** هذه العمليات تسمّع لحظياً في الـ Dashboard وتغير "الدخل الشهري" و "المصروف الشهري" و "الصافي".
3. **الربط بالذكاء الاصطناعي الشهري:** يعتمد الـ AI على هذه العمليات بشكل كامل عند توليد تقريره الشهري، بحيث يلاحظ بدقة أنماط الإنفاق السريع أو الاشتراكات التلقائية، ويوجه نصائح للتوفير.
4. **المحرك الهجين (Zero-Cost):** لا يعتمد النظام على الـ AI بشكل كامل. تم تطوير "محرك قواعد" (Rule Engine) ضخم يستطيع قراءة أكثر من 95٪ من رسائل البنوك المصرية ومحافظ الموبايل (Vodafone Cash, InstaPay, Orange Money) وحتى الرسائل الإنجليزية (مثل IBIN transferred sent). الـ AI يُستخدم كـ "طبقة أخيرة" فقط في حالة عجز المحرك عن فهم الرسالة.
5. **التحكم وصلاحيات الباقات:** يتوفر للمدير عبر لوحة التحكم ضبط سقف الرسائل لكل باقة. افتراضياً: باقة مجانية (5 عمليات فقط لتجربة الميزة)، باقة برو وألترا (غير محدود).

#### دليل إعداد المستخدم لتلقي الإشعارات (للتجربة)

**لمستخدمي iOS (Apple Shortcuts):**
قمنا بتسهيل العملية لتصبح ضغطة زر بدلاً من الإعداد اليدوي المعقد، من خلال "Shortcut جاهز":
1. افتح تطبيق SmartSpend واذهب إلى إعدادات الحساب > **"ربط الآيفون التلقائي (iOS Shortcut)"**.
2. **الخطوة 1 (التوكن):** اضغط على **"إنشاء Token"** (إن لم يكن موجوداً) وقم بنسخه.
3. **الخطوة 2 (التثبيت):** اضغط على زر **"Connect iPhone"** لفتح رابط iCloud الخاص بالـ Shortcut الجاهز. عند فتحه، اضغط **Set Up Shortcut**، وقم بعمل Paste للـ Token الذي نسخته.
4. **الخطوة 3 (الأتمتة):** في نفس صفحة الإعدادات، اكتب **الكلمة المميزة لرسالة البنك** (مثلاً `EGP` أو `البنك الأهلي`) واضغط حفظ.
5. افتح تطبيق **Shortcuts** في الآيفون، اذهب لتبويب **Automation** بالأسفل، وأنشئ أتمتة جديدة باختيار **Message**.
6. في خانة **Message Contains** اكتب الكلمة التي حفظتها، وتأكد من اختيار **Run Immediately**.
7. اضغط Next، واختر الـ Shortcut الجاهز الذي قمت بتثبيته.
(الآن، أي رسالة بنكية تحتوي على هذه الكلمة سيتم التقاطها وتسجيلها في حسابك لحظياً!)

**لمستخدمي Android (MacroDroid / Tasker):**
1. قم بإنشاء Macro جديد يقرأ رسائل SMS الواردة من أسماء معينة (البنك).
2. قم بعمل HTTP Request من نوع POST للرابط الخاص بالنظام، وضع الـ Token المنسوخ من التطبيق كـ Bearer Token، وأرسل نص الرسالة كـ JSON.

#### بخصوص محفظة Apple Pay
- تم تهيئة الـ Rule Engine لالتقاط رسائل **Apple Pay** بشكل مباشر دون الحاجة لطريقة ربط مختلفة.
- إذا قام المستخدم بالشراء عبر أبل باي، تأتي الرسالة غالباً بصيغة "Contactless payment" أو تحتوي على "Apple Pay". محرك القواعد سيفهم ذلك مباشرة ويصنف العملية כـ "مدفوعات" ويسجلها في التطبيق بشكل أوتوماتيكي ومجاني دون الحاجة لتدخل الذكاء الاصطناعي.
