...
| Settings screens | `src/components/admin/settings/` | Plan limits, API keys, discount codes |
...
| Settings | Every setting of `api/lib/system-settings-registry.ts` with secrets masked; per plan, in one tab each for Free, Pro and Ultra, the limits (tokens, daily entries, voice, bank messages saved on their own each month — later ones wait for the user as suggestions —, offline items, active goals, AI calls a minute, receipt and goal token caps, reports) and the feature switches of `contracts/plan-features.ts`; chat and voice-call limits per plan; the voice call's kill switch, models, daily cost caps and calls dashboard ([voice calls](voice-calls.md#in-the-admin-console)); a note that the parse model is chosen in the providers tab (the per-plan token-range routing editor is gone, [AI platform](ai-platform.md)); API keys with a live check; discount codes; a settings "backup" | `admin.getSettings`, `admin.updateSettings`, `admin.getAvailableModels`, `admin.validateApiKey`, `admin.getDiscountCodes`, `admin.createDiscountCode`, `admin.deleteDiscountCode`, `admin.triggerBackupDemo`, `voice.adminStats` |
