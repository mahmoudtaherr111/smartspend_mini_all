...
async function reset() {
  ...
  const [rows] = await connection.query(`SELECT id, user_id, profile_completed, JSON_LENGTH(onboarding_answers) as ac FROM user_profiles`);
  console.log("Reset complete. Current state:");
  rows.forEach(r => console.log(`  User ${r.user_id}: completed=${r.profile_completed}, answers=${r.ac}`));
